...
const missing = requiredStaticPages.filter((file) => !fs.existsSync(path.join(root, file)));
...
if (missing.length || duplicateProjectSlugs.length || duplicateArticleSlugs.length) {
  if (missing.length) console.error('Missing route files:', missing.join(', '));
  if (duplicateProjectSlugs.length) console.error('Duplicate project slugs:', duplicateProjectSlugs.join(', '));
  if (duplicateArticleSlugs.length) console.error('Duplicate article slugs:', duplicateArticleSlugs.join(', '));
  process.exit(1);
}

console.log(`Route verification passed: ${projectSlugs.length} projects, ${articleSlugs.length} articles.`);
